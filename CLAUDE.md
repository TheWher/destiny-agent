...
- **干支一律用引擎注入值，禁止前端/项目层按公历年自算**（2026-08-04 补）。曾踩坑：来因宫/年柱前端 `(birthYear-4)%10/12` 自算、校验器 `calc_sizhu(年,1,1)` 探测——都是公历年口径，年初立春前生人系统性错一年。生年干支由引擎立春分界算好（`plate.year_gz`）透传，前端/校验器只用该值，fallback 必须用完整出生日期解析
