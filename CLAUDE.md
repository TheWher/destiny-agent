...
**⑤ 独立信息源原则**：交叉验证的独立单位是信息源，不是换人。验证链至少一层吃独立于推导链的信息源（引擎实测/真盘/本人确认），判据：该层输入必须是推导链任何人都没碰过的原始数据。共享同一推导输入的多人验证 = 集体幻觉。**同模态验证不算独立验证，跨模态才算**（2026-08-12 卤/凶 案：视觉系两条读都向语义坍缩，像素几何检测才是独立裁判；OCR 的结论不能用 OCR 验，同一模态的多人验证 = 集体幻觉的模态镜像）。**票数≠独立性，分工侧分离才是**（2026-08-12 补）：三票机制站得住的前提是三侧错误空间两两不交——规则层死于脚本 bug、字典层死于文献歧义、口径死于文书不一致；同侧判据：问一句「会不会死于同一个错误」，同数据源/同脚本/同模型族都算同侧，两票共用一份数据 = 并成一票；审票先看侧、再看数；底层同模型族的多人背书不构成独立。**能脚本化的判定就脚本化**（2026-08-12 补）：可复现工件模型依赖最低、天然跨模态，unified_check.py 即「把判断变成脚本」实例；凡能脚本化的判定直接脚本化，不用每次重新论证独立性。
...
**⑧ 找方向先扫后说**：声称某语境/术语"最密、在 X 处"之前，先 grep 实测再发言；可机检的方向断言不做人脑接力（2026-08-12 s04 找方向教训："太微赋精解自化语境最密"凭印象说出，实测全文 0 处自化，候选全在格局/星曜页）。与③机检优先同一条绳。

**⑨ 先验后落（2026-08-12 补，s04 选句 FAIL 教训）**：定句/定稿必过机检，机检是落盘前最后一道闸。结论定了先跑 verifier/回归再落盘，禁止"定了就落、不验"；规则缺口可以补，流程习惯会复发。与⑧同一绳：先扫后说是发言前机检，先验后落是落盘前机检。

**⑩ 层声明（2026-08-12 补，SDZJ0170 定性回炉教训）**：扫描、统计、判据输入必须声明数据层（渲染层 md / 数据层 JSON / 刻本影像），不声明不采信。背景：SDZJ0170「简体转写」「8 文件全 0」均系渲染层 md 证据，数据层 JSON 繁体保真从未进过扫描，定性整体回炉（v3→v4，三票）。与「原始字符≠判断基数」（2.38→2.91 修正）同根，皆为扫错对象。快照完整性标注须写清「哪个层完整」：渲染层 complete ≠ 数据层 complete。**声明姿势（2026-08-12 补，密度数自带身份）**：密度数/读数一律带三维身份后缀 `[层|源|成员集版本]`（如 `61.89[古籍层|全层线聚合|mv4]`），跨工具公共格式——扫描脚本、指纹脚本、报告、changelog、文档五处同款；成员集版本号跨脚本同步（verify_fingerprint.py 与 scan_variant_report_r4.py 各一 MEMBER_SET_VERSION + TRUE_VARIANTS 成员集定义），升版须全部同步改，防版本漂移，脚本内已有同源注释。不声明层、或声明但无版本，均不采信。

**⑪ 古籍判据分层 + 回图前置（2026-08-12 定，SDZJ0170 刻本 OCR + 判据合并三方共识）**：底本接入五步序（2026-08-12 收口固化）：到货 → OCR 预案（质量闸探针：指纹句定位，定位失败触发回图）→ 探针体检（verify_fingerprint.py 内容级定位率 N/6）→ 定位率分流（逐句分配）→ 分层判据（下三层）。备料（交接包/OCR 预案/探针/指纹脚本）在等待期做足，到货即跑，不重新攒。古籍底本比对按三层判：① **内容级（强判据，直接定案）**：字词增删/句式，如「最宜制」无克字 → quanlan 制克增文实锤，不需要字形容差；② **字形级（单字退役）**：尅/剋 等价容差只用于「刻本 vs X」比对方向，禁止反向用掉——数据层全尅 vs quanlan 全剋 的「全归一」事实本身仍是转写证据，单字等价、转写习惯不等价；③ **分布/比例（弱信号挂账）**：尅:剋 分布、分布异文只输出倾向不输出结论。**回图前置（硬前提）**：OCR 文本层只做定位，字形级判定一律回原图 + 像素几何特征检测（开口/封口/顶横，unified_check.py 留档可复用），视觉读图降为辅助且须单字隔离脱离语境——视觉在语义语境下系统性坍缩（2026-08-12 卤/凶 案：『必有X禍』语境下视觉/OCR/转写者三层全向凶义坍缩，p66 4x 图视觉实测读出凶；像素检测须 OCR 定位行框 + pitch 对齐单字窗口 + ±0.25 偏移稳定性双查，band 合并测试被邻字污染导致两次结论翻转），禁止以 OCR 字符做字形结论。OCR 字形读数进判据前，先小样本像素标定判别力（卤/凶 案：235 处 OCR 输出开口闭口都读卤、零判别力，整体降级为定位工具；与 ⑩「原始字符≠判断基数」同根，是其字形侧镜像）。实测：OCR 系统性错字（尅→过/鼓/鞋、緊→聚、㐫→卤、一→千）连内容级都击穿，指纹句文本层 0/6 vs 回图 6/6 定位——两者不矛盾，正是质量闸「定位失败→触发回图」设计的正面验证；OCR 烂的底本老实全走图，OCR 干净才可先文本层筛；中间态按句分配不设全有/全无——以 verify_fingerprint.py 内容级定位率 N/6 为体检指标（脚本一物两用，不另造工具），N 随篇目覆盖浮动（覆盖探测：每篇目特征词简繁双形态；覆盖 0 篇=不适用、1 篇=覆盖不足、≥3 句且命中<3=毙、含 OCR 错字组合=走回图——0/6 三语义歧义 2026-08-12 实撞：渲染层 juan1 夹昌夹曲误标异文实为句子在卷二），命中的句子文本层筛、未命中的句子单独回图，体检指标与回图范围直接挂钩。第三底本归类顺序：先文本层筛内容差（又强又便宜），全中才挑 2-3 点回图验字形（又弱又贵）。刻本系识别指纹权重：制无克单独定案，沖/凶/于 支撑，尅:剋 混用最弱，部分命中读亲疏度不当二分法。尅:剋 混用规律零成本观察（翻页顺手记「此页尅/剋在哪」）：字段必带 页码+正文/注解+篇名 三件套——页码防攒出只有翻得到的页的偏样本；篇名防跨篇借样本（现样本：太微赋正文尅、骨髓赋正文尅、太微赋解注剋，验「正文尅注解剋」须用骨髓赋自身注解位，跨篇借样本有系统误差，分不清版式分工还是篇间差异）。混用结构按类型分类挂账：位置分工型（尅/剋：正文尅/解注剋）vs 字级零散型（卤/凶：同页并存、粒度到字，p18 入廟不凶闭口 0.42-0.56 / 倒限最卤偏开 0.23-0.41 同页两形），跨字形对比才有基础。

**⑫ 俗写归字三合一（2026-08-12 晚定，SDZJ0170 㐫/卤 案三方共识）**：① **归字分歧探针（挂名沉淀）**：整理团队的归字分歧「同句异字」= 刻本俗写模糊的天然标记，𦉢/铃 等俗写疑字直接复用。分类：同句异字（上下文逐字对得上、仅该字异）→ 俗写信号；异句异字（上下文对不上）→ 版本异文，与俗写无关；自动化先对上下文再报分歧。前提：异本必须真独立（别家抄同一电子文本者无信息量）。三来源缺一不可：King 字义纠正（㐫=凶异体，第一块砖）+ 春鳥橋 异本对照（同句识典归卤、别家归凶）+ Mose 𦉢/铃 文本层未归一兜底。② **定标条款（字形级判据基准）**：基准字形从同底本书影抠取，现代字体渲染只作参照物、不作真值（2026-08-12 㐫/卤 案教训：拿 SimSun 渲染的标准㐫 当 0.98 基准量刻本俗写，把加画俗写误判成卤）；阈值按页/按卷重定标（墨色/残损/扫描亮度逐页变），禁止全局单阈值；内部聚类自洽优先于外部比对（执行见③b 簇数检查）。③ **聚类终审流程**：a) 噪声前置：数簇前先滤图像噪声（墨晕/残损/扫描亮度劈出的碎簇先并掉或标疑），排除噪声再谈形态差异；b) 簇数检查：单簇 = 同一种刻法收工；多簇 = 混形（刻工轮换/真混排），逐簇对字典、逐簇定案；c) 书证验源：簇心对字典前先验书证出处，防「刻本证刻本」循环论证。已验：㐫=凶俗字书证 A00323 组 =《四声篇海》「音凶，俗字」/《字鉴》/《正字通》+ 唐碑别字（韦塤墓志、赵晃墓志），均独立字书碑刻、非明刻斗数系统，循环论证风险划掉；具体加画形态定案时书证再细看；d) 解释层回写：定案后特征数据不动，仅改「形态类↔字种」映射解释层；旧解释留档不覆盖，审计可溯。④ **429 挂账状态**：从「方向明确：转写归一错误」降为「待终审：俗写转岔，识典归㐫 大概率正确」（King 字义纠正翻案）。草案原文：docs/草案_俗写归字三合一_20260812.md
