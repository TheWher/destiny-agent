...
手动轨：python test_orchestrator.py（exit 0 = 全绿）
pytest 轨：模块级执行体已包进 _run_all()，import 不再自动跑；
          显式拆出的 4 个 error-path 用例（test_error_path_*）供 pytest 收集。
          （TODO-PAIPAN-PYTEST 参数化，2026-08-06 mose 落）
"""
...
# ══════════════════════════════════════════════════════════
# 手动轨执行体（2026-08-06 mose 落：整段包进 _run_all，
# 消除模块级 sys.exit 导致的 pytest 收集 INTERNALERROR）
# ══════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════
# pytest 轨：4 个 error-path 用例显式拆出（TODO-PAIPAN-PYTEST，2026-08-06 mose 落）
# 对应 _run_all 第 5 节/第 7 节里故意触发异常的错误路径测试：
# 此前框架在 except 分支 traceback.print_exc() 打脏 stderr（d253036 已改为塞 traceback 字段），
# 这 4 条拆成独立 pytest 用例，失败单独红牌，不再混在手动轨输出里。
# ══════════════════════════════════════════════════════════
...
def test_error_path_route_without_plate_dict():
    """route 不给 plate_dict：能力缺必填参数，框架应捕获并返回失败"""
    result = _new_orch().route("帮我排八字")
    assert not result.success, f"expected failure, got success={result.success}"
    assert result.error, "expected error message"


def test_error_path_run_bogus_plate_dict():
    """run 塞废 dict：底层 _build_year_lookup_table 崩，框架应兜底返回 CapabilityResult"""
    result = _new_orch().run("bazi_analysis", plate_dict={"test": True})
    assert isinstance(result, CapabilityResult)
    assert not result.success, f"expected framework-caught failure, got success={result.success}"


def test_error_path_route_minimal_plate_dict():
    """route 带最小 plate_dict（只有 input 无 pillars）：不崩，框架捕获"""
    result = _new_orch().route("帮我排八字", inject_tools=True,
                               plate_dict={"input": {"birth_datetime": "2005-08-19 01:35", "gender": "男"}})
    assert isinstance(result, CapabilityResult)


def test_error_path_route_no_inject_without_plate_dict():
    """route(inject_tools=False) 不给 plate_dict：同缺必填参数，框架捕获"""
