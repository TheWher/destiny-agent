...
- [ ] **来因宫收编三步**（形态查证 → King 判断题 → 收编实现）：
  ...
  3. **收编实现**：静态映射 → 表搬进引擎；宫干匹配制 → 表语义从“天干→宫名”换成“天干→地支位”（5 行：甲己戌/乙庚酉/丙辛申/丁壬未/戊癸午）+ 盘面映射层（地支位→当前盘宫名），测试覆盖整个换（mose 预判）。**顺带清理**（2026-08-04 代码排查）：前端 LAIYIN 两处 fallback 仍用 `(birthYear-4)%10` 公历年自算（凌晨干支铁律违反残留，year_gz 缺失时才触发）；routes/ziwei.py 盘指纹 `fp["laiyin"]` 查 tags["来因宫"] 但引擎从未打此 tag，恒空死代码——一并处理
