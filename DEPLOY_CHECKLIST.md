...
- [ ] **来因宫收编三步**（形态查证 → King 判断题 → 收编实现）：
  ...
  2. **映射值判断题**（形态结论后再做，**题干依赖形态，形态变了要重写**）：宫干匹配制下问“盘面来因宫显示：___，对 / 不对”（预期 = 生年干固定地支位 + 盘面映射后的宫名），**不问“甲年出父母宫对不对”**；静态制下才用旧题干。对 = 口径实证喂收编；不对 = 走异议分支（异议内容 → 原著定口径 → 再收编）。**证据边界**：判断题只能锁“宫干制大方向”，锁不住制式（King 盘甲年落在两制重合区）；制式判定只能靠书里己庚辛壬癸例盘，判断题数据不得当制式证据（否则是“拿默认值当确认”换马甲）。若真翻不到己庚辛壬癸例盘，制式定不死，收编实现留双套形态口子
  ...
  4. **查证行动化（翻书）**：梁湘润哪部著作讲来因宫，书名 King 选（他清楚手头有什么原著）。翻书三问（mose 定死，核对表已换双锚）：①来因宫怎么取 ②书里有没有例盘 ③例盘地支位对哪套锚——**优先找己庚辛壬癸年生人例盘（判别样本）**：己标巳位=个体制/标戌位=对首制，庚辰vs酉、辛卯/丑vs申、壬寅/子vs未、癸亥vs午；甲-戊例盘（戌酉申未午）两制同，只作“宫干制”方向证据。**补问**：若个体制，辛壬双候选位书里怎么消歧。King 按三问答结论，形态一次定死
