...
- [ ] **来因宫收编三步**（形态查证 → King 判断题 → 收编实现）：
  ...
  2. **映射值判断题**（形态结论后再做）：题干（King 对照生年干填）：生年干 ___ → 预期 ___ 宫（已定口径），盘面来因宫显示：___，**对 / 不对**。对 = 口径实证喂收编；不对 = 走异议分支（异议内容 → 原著定口径 → 再收编）
  3. **收编实现**：静态映射 → 表搬进引擎；动态落宫 → 前端表整块废弃，引擎写“生年干 → 匹配宫干 → 来因宫”计算
