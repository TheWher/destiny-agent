...
- [ ] **来因宫判断题**（hanako/mose：确认必须是主动核对，不能是扫一眼的默认值；判断题否定成本低，信号才硬）：
  - 题干（King 对照生年干填）：生年干 ___ → 预期 ___ 宫（梁湘润口径），盘面来因宫显示：___，**对 / 不对**
  - 预期值写进题干，King 只做核对，答哪边都是有效数据：对 = “前端表当前口径 ≈ 梁湘润口径”实证，喂收编决策；不对 = 走异议分支
  - 异议分支：异议内容 → 拉回梁湘润原著定口径 → 再收编（否则收编只是把有争议口径从前端挪进引擎，问题原封不动搬家）
