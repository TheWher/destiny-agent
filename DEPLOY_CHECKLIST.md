...
- [ ] **来因宫收编三步**（形态查证 → King 判断题 → 收编实现）：
  1. **形态查证（收编第一问，先于 King 的盘）**：来因宫取法是“静态固定映射”还是“动态宫干匹配”？初步查证信号（2026-08-04 网络检索）：主流钦天门/北派口径 = 命盘上宫干与生年干相同的宫位（动态，随盘面变），生年四化自该宫飞出；前端 LAIYIN 静态表（天干→宫名固定）形态上与主流不同。梁湘润具体取法须以原著或可靠二手查证，勿以网文定口径。**单盘判断题区分不了这两种形态**（King 只有一个盘，动态制下恰好落中静态值，答“对”只是碰巧），此问只能靠查证
     - **判别式校验尺**（hanako 推导 / mose 验证）：五虎遁定寅首后十二地支宫干分布只由生年干决定，宫干匹配制下“宫干=生年干”之位恒在固定地支：**甲己→戌、乙庚→酉、丙辛→申、丁壬→未、戊癸→午**。查证到任何口径先拿这组对：静态表不满足（天干→宫名固定）、落宫制不满足（无固定地支位）。同一地支位对应的宫名随时辰变（命宫位置动）——静态表原理上就覆盖不了所有人，若查证为宫干匹配制，“甲→父母”这类表只对特定时辰盘成立，直接判死刑，引擎写计算
  ...
  2. **映射值判断题**（形态结论后再做，**题干依赖形态，形态变了要重写**）：宫干匹配制下问“盘面来因宫显示：___，对 / 不对”（预期 = 生年干固定地支位 + 盘面映射后的宫名），**不问“甲年出父母宫对不对”**；静态制下才用旧题干。对 = 口径实证喂收编；不对 = 走异议分支（异议内容 → 原著定口径 → 再收编）
  3. **收编实现**：静态映射 → 表搬进引擎；宫干匹配制 → 表语义从“天干→宫名”换成“天干→地支位”（5 行：甲己戌/乙庚酉/丙辛申/丁壬未/戊癸午）+ 盘面映射层（地支位→当前盘宫名），测试覆盖整个换（mose 预判）。**已完成/待办清理**（2026-08-04 代码排查）：前端 6 处本命年干支自算 fallback 已全部删除（ziwei.html 878/912、ziwei-report.html 771/773/775/904，commit 待补），year_gz 缺失时来因宫显“—”、年柱显“?”，缺失即暴露，不再静默给错值；剩余 `(y-4)%10/12` 均为流年/大运公历口径（1-2 月窗口差一年，已记档待办）；routes/ziwei.py 盘指纹 `fp["laiyin"]` 为收编占位符，留注释“待引擎 laiyin 输出激活”，**复活时 tag 名必须与查询 key 严格对齐**（死代码复活最常见坑：key 差一个字，复活完还是死的）
  4. **查证行动化（翻书）**：梁湘润哪部著作讲来因宫，书名 King 选（他清楚手头有什么原著）。翻书三问定死（mose）：来因宫怎么取、书里有没有例盘、例盘地支位跟判别式五组（甲己戌/乙庚酉/丙辛申/丁壬未/戊癸午）对不对得上。King 按三问给结论，形态一次定死
