...
# 王亭之《太微赋》精解：丑宫太阴入庙而太阳失地，根本谈不上日月并明 → 日月并明判无。
# 但该盘实命中古籍『日月同臨』（《骨髓赋》『日月同临官居侯伯』注文『命安丑宫日月在未、命安未宫日月在丑谓之同临』），
# 机检实证（2026-08-05）日月同宫只出现在丑未。2026-08-05 King 指示待办现在做 → 拆台样本翻正：
# 日月并明判无不变，新增日月同臨=命中（同宫本名，非错名）。
DEMO_BIRTH = (2005, 1, 15, 0, 30, '男')
EXPECT_DEMO = {
    'year_gz': '甲申',  # 来源=引擎口径（构造盘，2026-08-05 引擎实测）
    '日月并明': False,   # 同宫丑被拆台（太阳失地）
    '明珠出海': False,   # 命非未
    '日月同臨': True,    # 2026-08-05 翻正：日月同宫丑+命丑，命中古籍同臨（原双判无）
}
