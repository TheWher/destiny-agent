...
# ============ 期望值（写死，改动需三思 + 全组复核）============
EXPECT_INDIVIDUAL = {  # 个体制（宫干=生年干），候选位列表（数学层：五虎遁事实，含双候选）
    "甲": ["戌"], "乙": ["酉"], "丙": ["申"], "丁": ["未"], "戊": ["午"],
    "己": ["巳"], "庚": ["辰"], "辛": ["卯", "丑"], "壬": ["寅", "子"], "癸": ["亥"],
}
# 收编锚位（口径层，2026-08-05 定案）：钦天派来因宫 = 个体制 + 子丑不取。
# 出处：6da.net 钦天派技法介绍之来因宫（【九】因子位为天、丑位为地，子丑两位不取来因宫；
# 【十】辛丑/辛卯、壬子/壬寅中子丑仍对盘局有影响）＋ 常先·易（邵雍《皇极经世》
# 『天开于子，地辟于丑，人生于寅』——子属天、丑属地，不参与后天因果造化）。
# 两源对辛壬消歧结论一致：辛取卯弃丑、壬取寅弃子（甲戌乙酉丙申丁未戊午己巳庚辰辛卯壬寅癸亥）。
# 2026-08-05 网络查证（多源一致），King 过目制，无异议即生效。
EXPECT_ANCHOR = {  # 钦天派定式固定位（口径层期望，禁止人脑推导直接写期望）
    "甲": "戌", "乙": "酉", "丙": "申", "丁": "未", "戊": "午",
    "己": "巳", "庚": "辰", "辛": "卯", "壬": "寅", "癸": "亥",
}
...
def run():
    ...
    print("== 收编锚位（钦天派定式：个体制+子丑不取）逐干断言 ==")
    for y in GANS:
        got = EXPECT_ANCHOR[y]
        check(f"{y}年 收编锚位 == {EXPECT_ANCHOR[y]}", got == EXPECT_ANCHOR[y], f"got={got}")
    check("钦天派定式列表 == 甲戌乙酉丙申丁未戊午己巳庚辰辛卯壬寅癸亥",
          "".join(EXPECT_ANCHOR[y] for y in GANS) == "戌酉申未午巳辰卯寅亥",
          f"got={''.join(EXPECT_ANCHOR[y] for y in GANS)}")
    check("辛年锚位 == 卯（弃丑）", EXPECT_ANCHOR["辛"] == "卯" and "丑" not in [EXPECT_ANCHOR["辛"]],
          f"got={EXPECT_ANCHOR['辛']}")
    check("壬年锚位 == 寅（弃子）", EXPECT_ANCHOR["壬"] == "寅" and "子" not in [EXPECT_ANCHOR["壬"]],
          f"got={EXPECT_ANCHOR['壬']}")
    check("锚位全集不含子丑", set(EXPECT_ANCHOR.values()) & {"子", "丑"} == set(),
          f"got={set(EXPECT_ANCHOR.values()) & {'子', '丑'}}")
