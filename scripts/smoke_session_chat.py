...
def main():
    ...
    try:
        # 1) 创建会话 → 磁盘持久化
        r = client.post("/api/ziwei/sessions", json={
            "title": "冒烟测试会话",
        ...
        # 2) 列表
        r = client.get("/api/ziwei/sessions")
        ids = [s["id"] for s in r.get_json()]
        assert sid in ids, "列表不含新会话"
        print(f"[2/6] 会话列表 OK，共 {len(ids)} 个")

        # 3) 重命名 → 磁盘同步
        r = client.put(f"/api/ziwei/sessions/{sid}", json={"title": "冒烟改名"})
        ...
        r = client.patch(f"/api/ziwei/sessions/{sid}", json={"messages": msgs})
        assert r.get_json().get("ok"), r.get_data(as_text=True)[:200]
        with open(disk, encoding="utf-8") as f:
        ...
        # 6) 删除 → 磁盘清理
        r = client.delete(f"/api/ziwei/sessions/{sid}")
        assert r.get_json().get("ok"), r.get_data(as_text=True)[:200]
